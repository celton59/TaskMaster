        ...
        <nav className="flex-1 px-2 py-0.5 flex flex-col overflow-hidden">
          <div className="space-y-0.5 px-1 flex-shrink-0">            
            ...
            <div>
              <Link 
                to="/tasks"
                className={cn(
                  "w-full flex items-center justify-between px-2 py-1.5 text-xs font-medium rounded-md",
                  location.includes('/tasks') ? "bg-primary-50 text-primary-700" : "text-neutral-700 hover:bg-neutral-50"
                )}
              >
                <div className="flex items-center">
                  <ListTodo className={`h-4 w-4 ${location.includes('/tasks') ? 'text-primary-600' : 'text-neutral-500'}`} />
          ...
          <div className="px-2 pt-0 flex-shrink-0">
            ...
            {categories.slice(0, 3).map((category) => (
              <div 
                key={category.id} 
                className="flex justify-between items-center px-2 py-1 text-xs font-medium rounded-md text-neutral-700 hover:bg-neutral-50 cursor-pointer transition-colors"
              >
                <div className="flex items-center">
                  <span className={`h-2 w-2 rounded-full ${getCategoryColor(category.color)} mr-2`}></span>
                  {category.name}
                </div>
                <Badge 
                  className="bg-neutral-100 border-0 text-[10px] font-normal h-4 text-neutral-600 hover:bg-neutral-100"
                >
            ...
            {categories.length > 3 && (
              <div className="px-2 py-1 text-[10px] text-primary-600 font-medium cursor-pointer hover:underline">
        ...
        <div className="p-2 border-t border-neutral-100">
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              ...
              <div className="ml-2">
                <p className="text-xs font-medium text-neutral-800">Admin Demo</p>
                <p className="text-xs text-neutral-500 text-[10px]">admin@example.com</p>
              </div>
            </div>
            <Button 
              size="icon" 
              variant="ghost" 
              className="h-6 w-6 rounded-full text-neutral-500 hover:text-neutral-700 hover:bg-neutral-100"
            >
...
function NavLink({ href, icon, label, isSubmenu = false }: NavLinkProps) {
  ...
  return (
    <Link 
      to={href}
      className={cn(
        "flex items-center px-2 py-1 text-xs font-medium rounded-md cursor-pointer transition-colors",
        isActive 
